...
$(document).ready(function() {
  ...
  // Creating an object to hold our artists.
  var artists = {
    "Philip Guston": {
      name: "Philip Guston",
      talent: 240,
      attack: 30,
      imageUrl: "assets/images/guston.jpg",
      enemyAttackBack: 15
    },
    "Salvador Dali": {
      name: "Salvador Dali",
      talent: 160,
      attack: 14,
      imageUrl: "assets/images/dali.jpg",
      enemyAttackBack: 17
    },
    "Pablo Picasso": {
      name: "Pablo Picasso",
      talent: 140,
      attack: 20,
      imageUrl: "assets/images/picasso.jpeg",
      enemyAttackBack: 12
    },
    "Chuck Close": {
      name: "Chuck Close",
      talent: 130,
      attack: 18,
      imageUrl: "assets/images/close.jpeg",
      enemyAttackBack: 25
    }
    //   "": {
    //     name: "",
  ...
  var attacker;
  // Populated with all the artists the player didn't select.
  var combatants = [];
  ...
  var renderCharacter = function(character, renderArea) {
    ...
    var chartalent = $("<div class='character-talent'>").text(character.talent);
    charDiv
      .append(charName)
      .append(charImage)
      .append(chartalent);
  ...
  // this function will load all the artists into the character section to be selected
  var initializeGame = function() {
    // Loop through the artists object and call the renderCharacter function on each character to render their card.
    for (var key in artists) {
      renderCharacter(artists[key], "#artists-section");
    }
  };
  ...
  $("#artists-section").on("click", ".character", function() {
    ...
    if (!attacker) {
      ...
      attacker = artists[name];
      // We then loop through the remaining artists and push them to the combatants array.
      for (var key in artists) {
        if (key !== name) {
          combatants.push(artists[key]);
        }
      }
  ...
  $("#attack-button").on("click", function() {
    ...
    if ($("#defender").children().length !== 0) {
      ...
      // Reduce defender's talent by your attack value.
      defender.talent -= attacker.attack * turnCounter;

      // If the enemy still has talent..
      if (defender.talent > 0) {
        ...
        // Reduce your talent by the opponent's attack value.
        attacker.talent -= defender.enemyAttackBack;

        // Render the player's updated character card.
        updateCharacter(attacker, "#selected-character");

        // If you have less than zero talent the game ends.
        // We call the restartGame function to allow the user to restart the game and play again.
        if (attacker.talent <= 0) {
          clearMessage();
          restartGame("You have been defeated...GAME OVER!!!");
          $("#attack-button").off("click");
        }
      } else {
        // If the enemy has less than zero talent they are defeated.
